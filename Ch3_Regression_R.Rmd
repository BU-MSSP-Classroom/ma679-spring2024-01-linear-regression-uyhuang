...
lmfit_2008<- lm(data = ames_raw_2008, 
                formula = log(SalePrice) ~ `Garage Area`+`1st Flr SF`+`Gr Liv Area`+`Full Bath`+`Year Built`+`Year Remod/Add`)
...
```         
The R-squared value is about 0.77, which indicates the model fits well.
```
...
```{r,echo=TRUE}
lmfit_reduced <- lm(data = ames_raw_2008, formula = log(SalePrice) ~ `Garage Area` + `1st Flr SF` + `Gr Liv Area` + `Year Built` + `Year Remod/Add`)
anova(lmfit_reduced, lmfit_2008)
```

Your answer: No, not all the predictors should be included. Here Full
Bath has a p-value=0.121\>0.05, so it is not statistically significant.
So I use Anova to compare with and without the predictor. From the
result we can see that the predictor doesn't provide much help in the
model.
...
```{r,echo=TRUE}
fitted_values <- predict(lmfit_2008)

residuals <- residuals(lmfit_2008)

plot(fitted_values, residuals, xlab = "Fitted Values", ylab = "Residuals",
     main = "Residuals vs Fitted")
abline(h = 0, col = "red") 
abline(v = mean(fitted_values), col = "blue") 
qqnorm(residuals)
qqline(residuals)
```
...
```         
Consdiering the background information, Year Built might have interaction with Year Remod/Add.
The residual plots seems to randomly spread with several outliers. There's no obvious signs indicating interaction exists.
```
...
```{r,echo=TRUE}
ames_predictors <- c("Garage Area", "1st Flr SF", "Gr Liv Area", "Full Bath", "Year Built", "Year Remod/Add")

poly_models <- list()

for(pred in ames_predictors) {

  formula <- as.formula(paste("log(SalePrice) ~ poly(`", pred, "`, 3, raw = TRUE)", sep=""))
  poly_models[[pred]] <- summary(lm(formula, data = ames_raw_2008))
}
...
```         
According to the fitted models, we can use the p-value to tell the linearity of each predictor.
1st Flr SF is linear.
Gr Liv Area is linear.
Full Bath is not linear.
Year Built is not linear.
Year Remod/Add is not linear.
```
...
```{r,echo=TRUE}
pred_2009<-exp(predict(lmfit_2008,newdata=ames_raw_2009))
ames_raw_2009$residual <- ames_raw_2009$SalePrice - pred_2009
ames_raw_2009 <- ames_raw_2009[order(ames_raw_2009$residual, decreasing = TRUE), ]
head(ames_raw_2009,5)
...
```{r,echo=TRUE}
ames_raw_2009 <- ames_raw_2009[order(ames_raw_2009$residual, decreasing = F), ]
head(ames_raw_2009,5)
...
lmfit_auto <- lm(mpg ~ cylinders + displacement + horsepower + weight+ acceleration + year + origin+name, Auto)
summary(lmfit_auto)
```
...
there's a linear relationship between weight and the response

ii. How strong is the relationship between the predictor and the
    response?

    The relationship between weight and the response are strong, and the
    others are not

iii. Is the relationship between the predictor and the response positive
     or negative?

     Negative, except displacement
...
plot(lmfit_auto, which = 1, main = "Residuals vs. Fitted")
```

Your answer:

```         
The residual plot is very spreaded, and it is randomly spreaded.
```
...
```{r,echo=TRUE}
Auto <- data.frame(lapply(Auto, as.numeric))
...
```{r,echo=TRUE}
Auto_numeric <- Auto[, !names(Auto) %in% 'name']
...
variable . This is indicated by the multiple R-squared value of 0.8215,
which suggests that approximately 82.15% of the variability in miles per
gallon can be explained by the model's predictors. The adjusted
R-squared value of 0.8182 accounts for the number of predictors in the
model and still indicates a strong relationship. Additionally, the
F-statistic is highly significant (p \< 2.2e-16), which suggests that at
least some of the predictors have a non-zero association with the
response.

iii. What does the coefficient for the year variable suggest?

     The coefficient for the **`year`** variable is 0.750773, which is
     statistically significant (p \< 2e-16). This suggests that, on
     average, each additional year is associated with an increase of
     about 0.75 miles per gallon in the fuel efficiency of the car,
     holding all other variables constant. This might be interpreted as
     cars becoming more fuel-efficient over the years.
...
```         
The residual plots suggest that there are several unsually large outliers. There's some unusually high leverage.
```
...
```{r,echo=TRUE}
lm_fit_interaction <- lm(mpg ~ weight * horsepower, data=Auto)
...
```         
I think that the effect of horsepower on mpg could depend on the weight of the car. A lighter car might get more additional miles per gallon from an increase in horsepower than a heavier car would.
```
...
fit_log <- lm(Sales ~ log(Price) + Urban + US, data=Carseats)
summary(fit_log)

fit_sqrt <- lm(Sales ~ sqrt(Price) + Urban + US, data=Carseats)
summary(fit_sqrt)

fit_square <- lm(Sales ~ I(Price^2) + Urban + US, data=Carseats)
summary(fit_square)
...
```         
The Price coefficient shows the expected change in sales for each unit increase in price. A negative coefficient means higher prices are associated with lower sales. For a binary variable like Urban, the coefficient tells us the difference in sales between urban and non-urban stores, assuming other factors are constant. The US coefficient indicates the sales difference between stores in the US and those outside, with all else being equal. These interpretations assume that the model's other assumptions are met.
```
...
$$
\text{Sales} = \beta_0 + \beta_1 \cdot \text{Price} + \beta_2 \cdot \text{Urban}_{\text{Yes}} + \beta_3 \cdot \text{US}_{\text{Yes}} + \epsilon
$$
...
```{r,echo=TRUE}
#
fit_small <- lm(Sales ~ Price + Urban + US, data=Carseats)
summary(fit_small)
```
...
```         
The model in (e) is slightly better than the models in (a), but none of them fit the model good.
```
...
conf_intervals <- confint(fit_small, level = 0.95)
conf_intervals
```
